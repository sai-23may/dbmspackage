...
try{
	...
	ResultSet rs=st.executeQuery("select sum(total) from cart where email= '"+email+"' and adress is NULL");
	while(rs.next())
	{
		total=rs.getInt(1);
	}
 %>
        ...
        <tbody>
        <% 
      	ResultSet rs1=st.executeQuery("select *from product inner join cart on product.id=cart.product_id and cart.email= '"+email+"' and cart.adress is NULL");
      	while(rs1.next()){
      	%>
          <tr>
          <%sno=sno+1; %>
